...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const statsResponse = await apiClient.get("/orders/dashboard-stats")
      if (statsResponse.data.status === "success") {
        setStats(statsResponse.data.data.item)
      }

      // Fetch recent orders
      const ordersResponse = await apiClient.get("/orders/recent?recent_orders_count=5")
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-[#444444]">Order Status Distribution</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
          ...
          {recentOrders.length > 0 ? (
            <div className="space-y-4">
              {recentOrders.map((order) => (
                <div key={order.id} className="flex items-center justify-between p-4 bg-[#f2f2f2] rounded-lg">
                  <div>
                    <p className="font-medium text-[#444444]">{order.distributor}</p>
                    <p className="text-sm text-[#ababab]">Order #{order.id}</p>
                  </div>
